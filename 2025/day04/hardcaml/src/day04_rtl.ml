...
let () =
  if width <= 0 || height <= 0
  then failwithf "Invalid grid size: %dx%d" width height ()
;;
...
let create_part1 scope ({ clock; clear; start; part2 = _; in_valid; in_char } : _ I.t) : _ O.t =
  ...
  let need_input_raw =
    running
    &: (r.value >=: r1) &: (r.value <=: r_h)
    &: (c.value >=: c1) &: (c.value <=: c_w)
  in
  ...
  let valid_center_raw =
    (r.value >=: r2) &: (r.value <=: r_h1)
    &: (c.value >=: c2) &: (c.value <=: c_w1)
  in
  ...
  compile
    [ sm.switch
        ...
        ; ( Running
          , [ when_ valid_center
                [ count <-- count.value +: (zero 31 @: accessible) ]
...
let create scope ({ clock; clear; start; part2; in_valid; in_char } : _ I.t) : _ O.t =
  ...
  let cur_cached_val =
    mux write_idx.value (Array.to_list (Array.map cached_vals ~f:(fun v -> v.value))) in
  let cur_cached_addr =
    mux write_idx.value (Array.to_list (Array.map cached_addrs ~f:(fun v -> v.value))) in
  let cur_cached_valid =
    mux write_idx.value (Array.to_list (Array.map cached_valid ~f:(fun v -> v.value))) in
  let cur_cached_row =
    mux write_idx.value (Array.to_list (Array.map cached_rows ~f:(fun v -> v.value))) in
  let cur_cached_col =
    mux write_idx.value (Array.to_list (Array.map cached_cols ~f:(fun v -> v.value))) in
  ...
  node_wr_addr_w <== mux2 load_write load_addr
                       (mux2 center_write center_addr cur_cached_addr);
  node_wr_data_w <== mux2 load_write node_data_init
                       (mux2 center_write dead4 cur_new_val);

  q_we_w <== (load_enq |: (peel_write &: cur_should_enq));
  q_wr_data_w <== mux2 load_enq q_data_load
                    (concat_msb [ cur_cached_row; cur_cached_col ]);
  ...
  compile
    [ sm.switch
        ...
        ; ( Peel_check
          , [ proc (List.concat (Array.to_list (Array.mapi neighbor_info
                ~f:(fun i (nr, nc, valid) ->
